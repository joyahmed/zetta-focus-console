...
interface ProfileModalProps {
  ...
  onClose: () => void;
  mode: 'create' | 'edit';
  profile?: Profile; // Required for edit mode
...
export function ProfileModal({ isOpen, onClose, mode, profile, onSubmit }: ProfileModalProps) {
      ...
      <div className="relative bg-zetta-card border border-zetta-border rounded-lg shadow-2xl w-[500px] max-h-[80vh] overflow-auto">
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          ...
          <div>
            ...
            />
            {mode === 'create' && (
              <p className="text-[10px] text-gray-600 mt-1">ID will be auto-generated from the name</p>
            )}
            {mode === 'edit' && profile && (
              <p className="text-[10px] text-gray-600 mt-1">ID: {profile.id}</p>
            )}
          </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              <label className="block text-xs text-gray-400 mb-1">Focus (min)</label>
              <input
                type="number"
                min={1}
                max={180}
            ...
            <div>
              <label className="block text-xs text-gray-400 mb-1">Short Break</label>
              <input
                type="number"
                min={1}
                max={60}
            ...
            <div>
              <label className="block text-xs text-gray-400 mb-1">Long Break</label>
              <input
                type="number"
                min={1}
                max={60}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs text-gray-400 mb-1">Motion Intensity</label>
              <select
                value={intensity}
          ...
          {/* Sound */}
          <div>
            <label className="block text-xs text-gray-400 mb-1">Ambient Sound</label>
            <select
              value={sound}
          ...
          <div className="flex justify-end gap-2 pt-2 border-t border-zetta-border">
            ...
            <button
              ...
              className={`px-4 py-2 text-xs text-white rounded transition-colors disabled:opacity-50 ${
                mode === 'edit' ? 'bg-amber-500 hover:bg-amber-600' : 'bg-blue-500 hover:bg-blue-600'
              }`}
            >
