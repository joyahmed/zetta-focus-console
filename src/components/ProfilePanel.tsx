...
export function ProfilePanel({
	...
	const getLicenseBadge = () => {
		...
		if (type === 'Trial' && days !== null) {
			if (isExpiring) {
				return { label: `Trial ${days}d`, bg: 'rgba(245, 158, 11, 0.15)', text: '#f59e0b', border: 'rgba(245, 158, 11, 0.3)' };
			}
			if (isWarning) {
				return { label: `Trial ${days}d`, bg: 'rgba(234, 179, 8, 0.15)', text: '#ca8a04', border: 'rgba(234, 179, 8, 0.3)' };
			}
			return { label: `Trial ${days}d`, bg: 'rgba(34, 197, 94, 0.15)', text: '#22c55e', border: 'rgba(34, 197, 94, 0.3)' };
		}

		const badges: Record<string, { label: string; bg: string; text: string; border: string }> = {
			Pro: { label: 'Pro', bg: 'rgba(59, 130, 246, 0.15)', text: '#3b82f6', border: 'rgba(59, 130, 246, 0.3)' },
			Founder: { label: 'Founder', bg: 'rgba(168, 85, 247, 0.15)', text: '#a855f7', border: 'rgba(168, 85, 247, 0.3)' },
			Free: { label: 'Free', bg: 'rgba(107, 114, 128, 0.1)', text: '#6b7280', border: 'rgba(107, 114, 128, 0.2)' },
		};
	...
	return (
		<div className='p-3 md:p-4 lg:p-6 bg-zetta-card border border-zetta-border rounded-lg h-full overflow-auto'>
			<h2
				className='text-xs md:text-sm font-medium uppercase tracking-wider mb-2 md:mb-4'
			...
			<div className='flex items-center gap-2 md:gap-3 mb-2 md:mb-4'>
				...
				<div className='flex-1'>
					<div className='flex items-center gap-2'>
						...
						{profile.is_preset ? (
							<span className='px-1.5 py-0.5 text-[9px] bg-blue-400/20 dark:bg-blue-500/20 text-blue-500 dark:text-blue-400 rounded'>
								PRESET
							</span>
						) : (
							<span className='px-1.5 py-0.5 text-[9px] bg-green-500/20 text-green-600 dark:text-green-400 rounded'>
								CUSTOM
							</span>
						)}
						{/* License Badge */}
						<span
							...
							style={{
								backgroundColor: licenseBadge.bg,
								color: licenseBadge.text,
								borderColor: licenseBadge.border,
							}}
						>
							{licenseBadge.label}
						</span>
