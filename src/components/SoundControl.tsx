...
export function SoundControl({
  ...
  const handleVolumeChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        await invoke('execute_command', { command: `sound volume ${newVolume}` });
      } catch (error) {
  ...
  return (
    <div className="flex items-center gap-3 bg-zetta-bg/80 backdrop-blur-sm rounded-lg px-4 py-2 border border-zetta-border/50 shadow-lg">
      ...
      >
        {isPlaying ? (
          <svg className="w-5 h-5 text-zetta-glow" fill="currentColor" viewBox="0 0 20 20">
            <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8 7a1 1 0 00-1 1v4a1 1 0 001 1h4a1 1 0 001-1V8a1 1 0 00-1-1H8z" clipRule="evenodd" />
          </svg>
        ) : (
          <svg className="w-5 h-5 text-gray-400 hover:text-zetta-glow" fill="currentColor" viewBox="0 0 20 20">
