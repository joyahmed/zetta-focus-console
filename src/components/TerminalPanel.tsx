...
export function TerminalPanel({ onCommand, onHelp, isFocused, onFocus, onBlur, sessionSummary, onSummaryRead }: TerminalPanelProps) {
  const [history, setHistory] = useState<TerminalLine[]>([
    { type: 'output', content: 'Zetta Focus Console v1.0.0' },
    { type: 'output', content: 'Type "help" for available commands.' },
    { type: 'output', content: 'Press Ctrl+K to focus terminal, ESC to blur.' },
  ]);
  ...
  return (
    ...
    >
      <div className="px-2 md:px-4 py-1.5 md:py-2 border-b border-zetta-border bg-zetta-bg/50 flex items-center justify-between">
        ...
        {isFocused && (
          <span className="text-xs text-blue-400">Focused</span>
        )}
      </div>
