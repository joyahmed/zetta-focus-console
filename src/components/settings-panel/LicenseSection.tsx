...
const LicenseSection = ({
	...
	const [selectedProduct, setSelectedProduct] = useState<
		'pro' | 'founder'
	>('pro');
	const [proPricing, setProPricing] = useState<PricingInfo | null>(
		null
	);
	const [founderPricing, setFounderPricing] =
		useState<PricingInfo | null>(null);
	const [checkoutInfo, setCheckoutInfo] =
		useState<CheckoutInfo | null>(null);
	...
	useEffect(() => {
		const fetchPaymentData = async () => {
			try {
				const proPrice = await invoke<PricingInfo>(
					'payment_get_pricing',
					{ productType: 'pro' }
				);
				setProPricing(proPrice);

				const founderPrice = await invoke<PricingInfo>(
					'payment_get_pricing',
					{ productType: 'founder' }
				);
	...
	const handleCheckout = async (
		provider: 'lemon_squeezy' | 'bkash'
	) => {
		setLoading(true);
		try {
			const info = await invoke<CheckoutInfo>(
			...
			if (provider === 'lemon_squeezy' && info.checkout_url) {
				await invoke('payment_open_checkout', {
					url: info.checkout_url
				});
			}
		} catch (error) {
	...
	const hasPro =
		currentLicense === 'Pro' || currentLicense === 'Founder';
										...
										>
											...
											{proPricing && (
												<div
													...
													style={{ color: 'var(--text-muted)' }}
												>
													{proPricing.currency_symbol}
													{proPricing.price_usd} USD
												</div>
											)}
											<div
										...
										>
											...
											{founderPricing && (
												<div
													...
													style={{ color: 'var(--text-muted)' }}
												>
													{founderPricing.currency_symbol}
													{founderPricing.price_usd} USD
												</div>
											)}
											<div
												className='text-xs mt-1'
											...
											<div className='flex items-center justify-between'>
												<div>
													...
													<div
														...
														style={{ color: 'var(--text-muted)' }}
													>
														Global payment - Credit card, PayPal,
														Apple Pay
													</div>
												</div>
												<svg
											...
											<div className='flex items-center justify-between'>
												<div>
													...
													<div
														...
														style={{ color: 'var(--text-muted)' }}
													>
														Bangladesh local payment - Manual key
														issuance
													</div>
												</div>
												<span
										...
										>
											<div>
												...
												<span className='font-mono font-semibold'>
													{checkoutInfo.bkash_info.merchant_number}
												</span>
											</div>
											<div>
												<span style={{ color: 'var(--text-muted)' }}>
												...
												<span className='font-semibold'>
													৳{checkoutInfo.bkash_info.amount} BDT
												</span>
											</div>
											<div
											...
											>
												Contact:{' '}
												{checkoutInfo.bkash_info.contact_email}
											</div>
										</div>
									</div>
								)}
